# MAGI & EVA Theme Suite 🧠💜💚🔴

A high-performance theme suite for VS Code, inspired by the **MAGI Supercomputer**, **EVA-01**, and **EVA-02** from *Neon Genesis Evangelion*.

## 🌓 Three Legendary Variants

### 1. MAGI (Logical Mode) 🧡
Inspired by the MAGI System. High-spec Orange primary colors with deep black backgrounds for maximum logical focus.

![MAGI](magi.png)

### 2. EVA-01 (Berserk Mode) 💜💚
The fierce colors of Unit-01. Deep Midnight Purple atmosphere with Neon Green and Vibrant Purple accents.

![EVA-01](EVA01.png)

### 3. EVA-02 (Production Model) 🔴🧡
Pure aggressive power. Deep Crimson highlights on a dark thermal base.

![EVA-02](EVA02.png)

## 🚀 Features

*   **Semantic Highlighting**: Optimized for deep code understanding.
*   **Git Decorations**: Sidebar integration for modified/added/deleted files.
*   **Bracket Pair Colorization**: High-visibility bracket cycling.
*   **Modern C++ Ready**: Specifically tuned for C++17 and modern web development.

## 📟 Screenshots

Additional showcase available in the [GitHub Repository](https://github.com/SinsuSquid/MAGI-theme).

## 🤖 System Status: ACTIVE

> "The MAGI System is now in complete agreement."

Created with 💖 by SinsuSquid.
